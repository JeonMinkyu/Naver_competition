rm(list = ls())
setwd("C:/Users/good/Desktop/naver") #디렉토리 설정
library(data.table)
...
data2 <- data2 %>% mutate(area_cat2 = ifelse(forestfire$area > 0, 1, 0))
...
confusionMatrix(table(predicted.prob, test$area1))$overall[1]
...
library(xgboost)
model.xg <- xgboost(data = data.matrix(train[,-13]), label = train[,13], max_depth = 4, nrounds = 30, objective = "binary:logistic" )
pred <- as.numeric(predict(model.xg, data.matrix(test[,-13])) > 0.5)
confusionMatrix(table(pred, test$area1))
